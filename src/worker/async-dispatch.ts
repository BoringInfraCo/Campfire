/**
 * Async dispatch for Workers/D1.
 *
 * Mechanical async port of `src/http/dispatch.ts`: identical validation and
 * method mapping over `AsyncCampfireService`. The acting actor still comes
...
export async function dispatchCampfireMethodAsync(
  ...
  switch (method) {
    ...
    case "list_workspaces":
      return await service.listWorkspaces(ctx);
    case "create_workspace":
      return await service.createWorkspace(ctx, {
        teamId: str(params, "teamId"),
        name: str(params, "name"),
        description: optionalStr(params, "description"),
      });
    case "update_workspace":
      return await service.updateWorkspace(ctx, {
    ...
    case "get_activity": {
      ...
      return await service.getActivity(ctx, query);
    }
    case "join_workspace":
      // Role, if supplied, is ignored by the service; invite supplies the role.
      return await service.joinWorkspace(ctx, { workspaceId: str(params, "workspaceId") });
    case "invite_workspace":
      return await service.inviteToWorkspace(ctx, {
        workspaceId: str(params, "workspaceId"),
    ...
    case "register_agent_session": {
      ...
      return await service.registerAgentSession(ctx, input);
    }
    case "create_goal":
      return await service.createGoal(ctx, {
        workspaceId: str(params, "workspaceId"),
        title: str(params, "title"),
        description: optionalStr(params, "description"),
      });
    case "update_goal":
      return await service.updateGoal(ctx, {
    ...
    case "add_finding":
      return await service.addFinding(ctx, {
        workspaceId: str(params, "workspaceId"),
    ...
    case "update_task": {
      ...
      return await service.updateTask(ctx, {
    ...
    case "add_artifact":
      return await service.addArtifact(ctx, {
        workspaceId: str(params, "workspaceId"),
    ...
    case "issue_token":
      return await service.issueToken(ctx, requireActorRef(params));
    case "revoke_token": {
      ...
      return await service.revokeToken(ctx, target);
    }
    case "end_agent_session":
      await service.endAgentSession(ctx, str(params, "sessionId"));
      return { ok: true };
    default: {
